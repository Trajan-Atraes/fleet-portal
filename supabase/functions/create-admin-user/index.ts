...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: adminRow } = await supabaseAdmin
      .from("admins").select("id, is_super").eq("id", caller.id).maybeSingle();
    ...
    // Audit log: super admin created an admin
    await supabaseAdmin.from("audit_logs").insert({
      table_name: "admins", record_id: newUser.user.id, action: "user_create",
      new_data: { email, role: "admin" },
      changed_by: caller.id, user_email: caller.email, category: "ADMIN",
      metadata: { created_by_edge_function: "create-admin-user" },
    }).catch(() => {});
