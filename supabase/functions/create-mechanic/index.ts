...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { error: linkErr } = await supabaseAdmin
      .from("mechanics")
      .insert({ id: newUser.user.id, email, name });

    if (linkErr) throw linkErr;

    // Audit log: admin created a mechanic
    await supabaseAdmin.from("audit_logs").insert({
      table_name: "mechanics", record_id: newUser.user.id, action: "user_create",
      new_data: { email, name, role: "mechanic" },
      changed_by: caller.id, user_email: caller.email, category: "ADMIN",
      metadata: { created_by_edge_function: "create-mechanic" },
    }).catch(() => {});
